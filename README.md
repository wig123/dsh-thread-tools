...
Eleven checks, all passing against a profile whose session store held 274 sessions across 7 projects:
...
- **Delivery is one-way.** There is no wait-for-reply tool: the target's answer lands in the target's own session and in its log.
