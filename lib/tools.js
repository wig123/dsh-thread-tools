...
import { collectThreads, isTopLevelSession, listSessionHeaders, searchThreads, servicesFor, } from './session-state.js';
...
const THREAD_REPLY_SCHEMA = {
    ...
    properties: {
        ...
        targetSessionId: { type: 'string', required: true },
        text: { type: 'string', description: 'Text of the target latest assistant message.' },
        truncated: { type: 'boolean', description: 'Whether the text was cut at the deployment bound.' },
        turn: { type: 'integer', description: 'Turn the reply belongs to.' },
        seq: { type: 'integer', description: 'Event sequence of the reply.' },
    },
};
...
export function createThreadToolDefinitions(config) {
    ...
    const replyTool = defineTool({
        ...
        output: {
            ...
            render: (_args, value) => [{ type: 'text', text: renderReply(value) }],
        },
        async execute(args, exec) {
            const targetId = typeof args.session_id === 'string' ? args.session_id.trim() : '';
            if (targetId.length === 0)
            ...
            if (target === caller)
                return { status: 'self', targetSessionId: targetId };
            const header = (await listSessionHeaders(services.sessions, exec.signal)).find(entry => entry.id === target);
            if (header === undefined)
                return { status: 'unknown-session', targetSessionId: targetId };
            if (!isTopLevelSession(header))
                return { status: 'subagent-session', targetSessionId: targetId };
            ...
            return reply === undefined
                ? { status: 'no-reply', targetSessionId: targetId }
                : {
                    status: 'reply',
